...
const columns = [
    {
        ...
        render: (text, record) => (
            <span>
                <a onClick={beclick.bind(this, text)}>{getActionTitle(text)}</a>
            </span>
        )
    },
    {
        ...
        return (
            <div className="free-bill-manager">
                <div className="head">
                    ...
                    <Button type="ghost" onClick={this.queryClick} style={{ margin: "0 10px"}}>查询</Button>
                    <Button type="ghost" onClick={this.exportClick}>导出</Button>
                </div>
                <Table bordered rowKey="id" style={tbstyle} rowSelection={rowSelection} dataSource={this.state.dataSource}
